  ...
  return (
    <div className="space-y-4 pb-6">
      ...
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-xs space-y-3">
        ...
        <div className={`${showFilters ? 'block' : 'hidden sm:block'} pt-2 border-t border-slate-100 flex items-center justify-end`}>
            ...
            <div className="flex flex-col min-[1300px]:flex-row min-[1300px]:items-center gap-3 bg-slate-50/80 px-3.5 sm:px-4 py-2.5 rounded-xl border border-slate-200/80 text-xs font-semibold text-slate-700 w-full min-[1300px]:w-auto">
              <div className="flex flex-wrap items-center gap-2 shrink-0">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="relative flex items-center">
                    <span className="absolute left-2.5 text-slate-400 font-medium text-xs">$</span>
                    <input type="number" min={minPrice + 1} max={100000} value={maxPrice}
                      onChange={(e) => setMaxPrice(Math.min(100000, Math.max(Number(e.target.value) || minPrice + 1, minPrice + 1)))}
                      className="w-20 sm:w-24 pl-5 pr-1.5 py-1 text-xs font-extrabold text-slate-800 bg-white border border-slate-300 rounded-lg focus:outline-none focus:border-indigo-500 shadow-2xs [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                  </div>
                </div>
              ...
              <div className="relative w-full min-[1300px]:w-52 min-[1450px]:w-64 h-6 flex items-center select-none py-1">
                ...
                <div className="absolute h-1.5 bg-slate-700 rounded-full transition-all duration-75 ease-out"
                  style={{ left: `${(minPrice / 100000) * 100}%`, width: `${((maxPrice - minPrice) / 100000) * 100}%` }} />
                <input type="range" min="0" max="100000" step="100" value={minPrice}
                  onChange={(e) => setMinPrice(Math.min(Number(e.target.value), maxPrice - 100))}
                  className={`absolute w-full h-1.5 appearance-none bg-transparent pointer-events-none focus:outline-none [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-slate-700 [&::-webkit-slider-thumb]:shadow-md [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:cursor-pointer [&::-moz-range-thumb]:pointer-events-auto [&::-moz-range-thumb]:w-4 [&::-moz-range-thumb]:h-4 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-white [&::-moz-range-thumb]:border-2 [&::-moz-range-thumb]:border-slate-700 [&::-moz-range-thumb]:shadow-md [&::-moz-range-thumb]:cursor-pointer ${minPrice > 50000 ? 'z-40' : 'z-25'}`}
                />
                <input type="range" min="0" max="100000" step="100" value={maxPrice}
              ...
              <div className={`relative bg-slate-100/80 overflow-hidden ${viewMode === 'list' ? 'sm:w-48 h-48 shrink-0' : 'aspect-[4/3] w-full'}`}>
                ...
                <div className="absolute top-2.5 left-2.5 flex flex-col gap-1 z-10">
                  {listing.discountPercent && listing.discountPercent > 0 && (
                    <span className="bg-rose-600 text-white text-[10px] font-black px-2 py-0.5 rounded-lg tracking-wide shadow-md flex items-center gap-1 animate-pulse">
