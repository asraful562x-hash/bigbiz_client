...
import { mapRoleNameToUserRole } from '../context/AppContext';
...
export const SellerProfileModal: React.FC<SellerProfileModalProps> = ({
  ...
  const localSeller = initialSeller
    || users?.find(u => String(u.id) === String(sellerId))
    || (currentUser && String(currentUser.id) === String(sellerId) ? currentUser : null);

  // The viewed user may not be in the local users list yet — pull their real
  // profile from the backend instead of showing the generic placeholder
  const [fetchedSeller, setFetchedSeller] = useState<User | null>(null);
  const [fetchAttempted, setFetchAttempted] = useState(false);

  useEffect(() => {
    if (localSeller || !sellerId || fetchAttempted) return;
    setFetchAttempted(true);

    const url = API_CONFIG.resolveUrl
      ? API_CONFIG.resolveUrl(API_CONFIG.ENDPOINTS.USERS.DETAIL(sellerId))
      : `/api/users/${sellerId}`;
    fetch(url)
      .then(r => (r.ok ? r.json() : null))
      .then((json: any) => {
        const u = json?.data || json;
        if (!u || (!u.id && !u.full_name && !u.email)) return;
        const resolvedRole = mapRoleNameToUserRole(u.role_name || u.role);
        setFetchedSeller({
          id: String(u.id),
          name: u.full_name || u.name || 'Member',
          username: `@${((u.email || '').split('@')[0] || 'member').toLowerCase().replace(/[^a-z0-9]/g, '')}`,
          email: u.email || '',
          avatar: u.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=400',
          role: resolvedRole,
          bio: u.bio || (u.company_name ? `Official account for ${u.company_name}` : ''),
          ...
          location: u.location || '',
          createdAt: u.create_date_time || 'Just now',
        } as User);
      })
      .catch(() => {});
  }, [localSeller, sellerId, fetchAttempted]);

  const seller = localSeller || fetchedSeller || {
    id: sellerId || 'unknown',
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto animate-in fade-in">
            ...
            <div className="flex items-center gap-2">
              {isOwnProfile ? (
                ...
                <>
                  ...
                  >
                    {isFollowing ? 'Γ£ô Following' : '+ Follow Shop'}
                  </button>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 text-slate-500 text-xs pt-1 border-t border-slate-200/60 font-medium">
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (onLikePost) onLikePost(p.id);
                          }}
                          className={`flex items-center gap-1.5 transition-colors cursor-pointer ${
                            p.isLiked ? 'text-rose-600 font-bold' : 'hover:text-rose-600'
                          }`}
                        >
                          <Heart className={`w-4 h-4 ${p.isLiked ? 'fill-current text-rose-600' : 'text-rose-500'}`} />
                      ...
                      {/* Expandable Comments in Profile Modal */}
                      {expandedComments[p.id] && (
                        <div className="bg-slate-100/70 p-3 rounded-xl space-y-2.5 mt-2 border border-slate-200/60">
                          {p.comments && p.comments.length > 0 ? (
                            <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                              {p.comments.map((c) => (
                                <div key={c.id} className="bg-white p-2 rounded-lg border border-slate-200 text-xs">
                          ...
                          {onCommentPost && (
                            ...
                            >
                              <input
                                ...
                                className="flex-1 text-xs px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500"
                              />
                              <button
                                type="submit"
                                className="bg-indigo-600 text-white p-1.5 rounded-lg hover:bg-indigo-700 transition-colors cursor-pointer"
                              >
                ...
                {sellerReviews.map((r) => (
                  <div key={r.id} className="p-3 bg-slate-50 rounded-xl text-xs space-y-1">
                    <div className="flex justify-between items-center font-bold text-slate-900">
                      ...
                      <span className="text-amber-500">{"Γÿà".repeat(r.rating)}</span>
                    </div>
      ...
      {/* Modern Confirmation Overlay Modal */}
      <ConfirmModal
        isOpen={Boolean(postToDelete)}
        title="Delete Post"
        message="Are you sure you want to delete this post? This will permanently remove its image attachments, comments, and data."
